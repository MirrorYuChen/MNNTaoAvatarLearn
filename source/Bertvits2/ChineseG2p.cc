...
ChineseG2P::ChineseG2P(const std::string &local_resource_root)
    : pinyin_(local_resource_root),
      word_spliter_(WordSpliter::GetInstance(local_resource_root)),
      tone_adjuster_(local_resource_root) {
  auto t0 = clk::now();
  LogInfo("ChineseG2P 开始初始化...");
  resource_root_ = local_resource_root;
  an2cn_ = An2Cn();

  auto pinyin_to_symbol_bin_path =
      resource_root_ + "common/text_processing_jsons/pinyin_to_symbol_map.bin";
  LoadBinToMap(pinyin_to_symbol_bin_path, pinyin_to_symbol_map_);

  auto hotwords_bin_path =
      resource_root_ + "common/text_processing_jsons/hotwords_cn.bin";
  ...
  LogInfo("ChineseG2P 初始化成功, timecost: {} ms", std::to_string(d1.count()));
}
...
void ChineseG2P::SaveHotwordsToBin(const std::string &filename) {
  ...
  for (const auto &pair : custom_phone_) {
    ...
    ofs.write(reinterpret_cast<const char *>(&hotword_size),
              sizeof(hotword_size));
    ofs.write(hotword.c_str(), hotword_size);

    // 写入声母
    size_t initials_size = initials.size();
    ofs.write(reinterpret_cast<const char *>(&initials_size),
              sizeof(initials_size));
    for (const auto &initial : initials) {
      size_t initial_size = initial.size();
      ofs.write(reinterpret_cast<const char *>(&initial_size),
                sizeof(initial_size));
    ...
    ofs.write(reinterpret_cast<const char *>(&finals_size),
              sizeof(finals_size));
    for (const auto &final : finals) {
      size_t final_size = final.size();
      ofs.write(reinterpret_cast<const char *>(&final_size),
                sizeof(final_size));
...
void ChineseG2P::ParsePinyToSymbolJsonFile(
    const std::string &pinyin_json_path) {
...
g2p_data ChineseG2P::ConvertInitialFinalToPhoneTone(
  ...
  for (size_t i = 0; i < initials.size(); ++i) {
    ...
    if (c == v) {
      ...
      if (!c.empty()) {
        // 多音节
        std::unordered_map<std::string, std::string> v_rep_map = {
      ...
      std::vector<std::string> phone =
          SplitString(pinyin_to_symbol_map_[pinyin], ' ');
...
std::string ChineseG2P::TextNormalize(const std::string &text) {
  ...
  LogDebug("g2p text_norm 输入: {}" + normalized_text);

  // 进行阿拉伯数字到汉字数字的转换
  normalized_text = TextAn2cn(normalized_text);
  LogDebug("g2p text_norm an2cn后: {}" + normalized_text);

  // 替换中文标点符号为英文标点符号
  normalized_text = ReplacePunctuations(normalized_text);
  LogDebug("g2p text_norm 替换标点后: {}" + normalized_text);
  return normalized_text;
}

g2p_data ChineseG2P::G2P(std::string &text) {
  LogDebug("g2p 输入句子: {}", text);

  std::vector<std::string> phones;
  std::vector<int> tones;
  std::vector<int> word2ph;

  // text是从原始句子中切出来的中文部分，可能包含标点符号，如
  // '你好啊.你是,我的男朋友?'
  // sentences是根据标点符号切分的小的分句，标点符号跟随前面的文本，如
  // ['你好啊.', '你是,', '我的男朋友?']
  auto segments = SplitChineseSentenceToParts(text);
  LogDebug("g2p 标点分句: {}", ConcatStrList(segments, "|"));
...
g2p_data ChineseG2P::G2PInternal(const std::vector<std::string> &segments) {
  ...
  // 对每个标点符号分割的小分句进行分词，并返回对应的词性，如 ['你好啊'] ->
  // [[(‘你好', 'l‘)， ('啊', 'zg')]]
  for (auto &seg : segments) {
    LogDebug(DELIMITER_LINE);
    LogDebug("g2p processing seg {}", seg);
    // 保存当前分句的声母和韵母
    ...
    LogDebug("g2p 分词结果: {}", ConcatList(seg_cut_list, "|"));

    // 计算拼音
    for (auto &[word, pos] : seg_cut_list) {
      LogDebug("g2p 开始处理词语: {}/{}", word, pos);
      // eng表示英文字母和数字
      if (pos == "eng") {
        LogDebug("g2p 是英文和数字: {}", word);
        continue;
      }
      // 如果词语在自定义词表里面，采用自定义的发音
      if (custom_phone_.find(word) != custom_phone_.end()) {
        LogDebug("g2p 是自定义词语: {}", word);
        auto [sub_initials, sub_finals] = custom_phone_[word];
        initials.insert(initials.end(), sub_initials.begin(),
                        sub_initials.end());
        finals.insert(finals.end(), sub_finals.begin(), sub_finals.end());
      } else {
        LogDebug("g2p 提取拼音: {}", word);
        auto [sub_initials, sub_finals] = pinyin_.Process(word);
        LogDebug("g2p 原始声母: {}", ConcatStrList(sub_initials, "|"));
        LogDebug("g2p 原始韵母: {}", ConcatStrList(sub_finals), "|");

        sub_finals = tone_adjuster_.Process(word, pos, sub_finals);
        LogDebug("g2p 修改声调后韵母: {}", ConcatStrList(sub_finals));
    ...
    LogDebug("g2p 该seg全部声母: {}", ConcatStrList(initials, "|"));
    LogDebug("g2p 该seg全部韵母: {}", ConcatStrList(finals, "|"));
    all_initials.insert(all_initials.end(), initials.begin(), initials.end());
    all_finals.insert(all_finals.end(), finals.begin(), finals.end());

    // 将拼音转换为音素和音调
    auto [cur_phones, cur_tones, cur_word2ph] =
        ConvertInitialFinalToPhoneTone(initials, finals);
    ...
    LogTrace(DELIMITER_LINE);
  }

  LogDebug("g2p 整句内容: {}", ConcatStrList(segments, "|"));
  LogDebug("g2p 整句声母: {}", ConcatStrList(all_initials, "|"));
  LogDebug("g2p 整句韵母: {}", ConcatStrList(all_finals, "|"));
  LogDebug("g2p 整句phones: {}", ConcatStrList(phones, "|"));
  LogDebug("g2p 整句tones: {}", ConcatIntList(tones, "|"));
  LogDebug("g2p 整句word2ph: {}", ConcatIntList(word2ph, "|"));

  return make_tuple(phones, tones, word2ph);
}

phone_data ChineseG2P::CleanedTextToSequence(const g2p_data &g2p_data_,
  ...
  LogDebug("g2p 调整后phones(size= {}):{}", std::to_string(int_phones.size()), ConcatIntList(int_phones, "|"));
  LogDebug("g2p 调整后tones(size={}):{}", std::to_string(adjusted_tones.size()), ConcatIntList(adjusted_tones, "|"));
  LogDebug("g2p 调整后word2ph(size={}):{}", std::to_string(word2ph.size()), ConcatIntList(word2ph, "|"));
  LogDebug("g2p lang_ids(size={}):{}", std::to_string(lang_ids.size()), ConcatIntList(lang_ids, "|"));

  return std::make_tuple(int_phones, adjusted_tones, lang_ids, word2ph);
}

std::tuple<std::string, phone_data>
ChineseG2P::Process(SentLangPair &sent_lang) {
  LogDebug("g2p 输入文本: {}", sent_lang.sent);
  auto norm_text = TextNormalize(sent_lang.sent);
  LogDebug("g2p norm后文本: {}", norm_text);
