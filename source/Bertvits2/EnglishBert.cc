...
NAMESPACE_BEGIN
EnglishBert::EnglishBert() = default;
...
EnglishBert::EnglishBert(const std::string &local_resource_root) {
  ...
  // 读取bert token json
  auto json_path =
      local_resource_root_ + "common/text_processing_jsons/en_bert_token.json";
  ...
  auto mnn_model_path =
      local_resource_root_ + "common/mnn_models/english_bert.mnn";
  module.reset(Module::load(input_names, output_names, mnn_model_path.c_str(),
                            rtmgr, &mdconfig));
  LogInfo("en_bert模型加载成功: {}", mnn_model_path);
}
...
EnglishBert::Process(const std::string &text, const std::vector<int> &word2ph) {
  ...
  auto output_ptr = outputs[0]->readMap<float>();
  int output_size = outputs[0]->getInfo()->size;
  LogDebug("en_bert 输出bert feature 维度: {}", std::to_string(output_size));
  float *tmp_ptr = (float *)output_ptr;
  float bert_sum = 0.0f;
  for (int i = 0; i < token_num; ++i) {
  ...
  // 根据word2ph对原始的网络输出进行复制，如word2ph=[2, 1,
  // 3]，则会对原始的【1024xN]的数据 的第二个维度依次复制2次，1次和3次
