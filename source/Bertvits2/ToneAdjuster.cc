...
NAMESPACE_BEGIN
ToneAdjuster::ToneAdjuster(const std::string &local_resource_root)
    : word_spliter_(WordSpliter::GetInstance(local_resource_root)) {
  auto t0 = clk::now();
  LogInfo("ToneAdjuster 开始初始化...");
  ...
  auto json_path =
      resource_root_ + "common/text_processing_jsons/default_tone_words.json";
  ParseDefaultToneWordJson(json_path);

  auto t1 = clk::now();
  auto d1 = std::chrono::duration_cast<ms>(t1 - t0);

  LogInfo(" ToneAdjuster 初始化完成, timecost: {} ms",
          std::to_string(d1.count()));
}
...
void ToneAdjuster::ParseDefaultToneWordJson(const std::string &json_path) {
  ...
  must_neural_words_ =
      json_obj["must_neural_tone_words"].get<std::vector<std::string>>();
  not_neural_words_ =
      json_obj["must_not_neural_tone_words"].get<std::vector<std::string>>();

  LogDebug("tone_adjuster 必须轻声的词语: {}",
           ConcatStrList(must_neural_words_, "|"));
  LogDebug("tone_adjuster 不能轻声的词语: {}",
           ConcatStrList(not_neural_words_, "|"));
}

// 判断一个字符串是否以指定前缀开头
bool ToneAdjuster::StartsWith(const std::string &str,
...
bool ToneAdjuster::HanziIn(const std::string &hanzi,
                           const std::vector<std::string> &pattern) {
  return std::find(pattern.begin(), pattern.end(), hanzi) != pattern.end();
}

// 判断一个词语是否在所给的模式中，如 PhraseIn("阿里巴巴", {"北京", “阿里巴巴",
// "他"}) = true
bool ToneAdjuster::PhraseIn(const std::string &hanzi,
                            const std::vector<std::string> &pattern) {
  return std::find(pattern.begin(), pattern.end(), hanzi) != pattern.end();
}

// 判断一个词语的某一部分是否在所给的模式中，如 PhrasePartIn("阿里巴巴", 2, 2,
// {"北京", “巴巴", "他"}) = true
bool ToneAdjuster::PhrasePartIn(const std::string &word, int start_index,
                                int size,
                                const std::vector<std::string> &pattern) {
  ...
  return std::find(pattern.begin(), pattern.end(), target_phrase) !=
         pattern.end();
}

// 判断词性是否在所在的模式中，如 PosIn("n", {"a", "n", "v"}) = true
// 注意词性可能是多个字母，因此采用char类型表示不合适，应该采用std::string
bool ToneAdjuster::PosIn(const std::string &pos,
                         const std::vector<std::string> &pattern) {
  return std::find(pattern.begin(), pattern.end(), pos) != pattern.end();
}

// 寻找词语中的第一个字在原始句子中的位置索引
int ToneAdjuster::FindSubstring(const std::vector<std::string> &str_list,
...
int ToneAdjuster::FindSubstring(const std::string &str,
...
ToneAdjuster::NeuralSandhi(const std::string &raw_word, const std::string &pos,
                           std::vector<std::string> &finals) {
  auto word = SplitUtf8String(raw_word);
  // 输入文本中没有汉字，直接返回
  if (word.size() < 1) {
    LogWarn("tone_adjuster NeuralSandhi 没有中文: {}", raw_word);
    return finals;
  }

  // 处理叠词，例如奶奶, 试试, 旺旺，爸爸等，将第二个字转换为轻声
  for (size_t j = 0; j < word.size(); ++j) {
    if (j >= 1 && word[j] == word[j - 1] && (PosIn(pos, {"n", "a", "v"})) &&
        PhraseIn(raw_word, not_neural_words_)) {
      LogTrace("tone_adjuster NeuralSandhi 叠词: {}", raw_word);
      finals[j].back() = '5';
    }
  }
  ...
  if (word.size() >= 1 && PhrasePartIn(raw_word, -1, 1, yuqici_pattern_)) {
    LogTrace("tone_adjuster NeuralSandhi 语气词: {}", raw_word);
    finals.back().back() = '5';
  }
  // 对"的地得"进行处理
  else if (word.size() >= 1 && PhrasePartIn(raw_word, -1, 1, de_pattern_)) {
    //        LogTrace("tone_adjuster NeuralSandhi 的地得: " + raw_word);
    // TODO 这里会有问题，例如 ‘机会难得’，
    //        finals.back().back() = '5';
  } else if (word.size() > 1 && PhrasePartIn(raw_word, -1, 1, men_pattern_) &&
             PosIn(pos, {"r", "n"}) && PhraseIn(raw_word, not_neural_words_)) {
    LogTrace("tone_adjuster NeuralSandhi 子们: {}", raw_word);
    finals.back().back() = '5';
  } else if (word.size() > 1 && PhrasePartIn(raw_word, -1, 1, shang_pattern_) &&
             PosIn(pos, {"s", "l", "f"})) {
    LogTrace("tone_adjuster NeuralSandhi 上下里: {}", raw_word);
    finals.back().back() = '5';
  } else if (word.size() > 1 && PhrasePartIn(raw_word, -1, 1, lai_pattern_) and
             PhrasePartIn(raw_word, -2, 1, lai_aux_pattern_)) {
    LogTrace("tone_adjuster NeuralSandhi 来去: {}", raw_word);
    finals.back().back() = '5';
  } else if ((ge_idx >= 1 &&
              (IsChineseDigital(raw_word, ge_idx - 1) ||
               PhrasePartIn(raw_word, ge_idx - 1, 1, ge_aux_pattern_))) ||
             raw_word == "个") {
    LogTrace("tone_adjuster NeuralSandhi 个: {}", raw_word);
    finals[ge_idx].back() = '5';
  } else {
    if (PhraseIn(raw_word, must_neural_words_) ||
        (word.size() > 2 &&
         PhrasePartIn(raw_word, word.size() - 2, 2, must_neural_words_))) {
      LogTrace("tone_adjuster NeuralSandhi 必须轻声的词: {}", raw_word);
      finals.back().back() = '5';
    }
  }
  ...
  if (word_list.size() < 1) {
    LogWarn("tone_adjuster NeuralSandhi 没有汉字: {}", raw_word);
    return finals;
  }
  ...
  finals_list.push_back(std::vector<std::string>(
      finals.begin(), finals.begin() + first_subword_length));
  finals_list.push_back(std::vector<std::string>(
      finals.begin() + first_subword_length, finals.end()));

  for (size_t i = 0; i < word_list.size(); ++i) {
    auto cur_word_utf8 = SplitUtf8String(word_list[i]);
    if (PhraseIn(word_list[i], must_neural_words_) ||
        (cur_word_utf8.size() >= 2 &&
         PhrasePartIn(word_list[i], cur_word_utf8.size() - 2, 2,
                      must_neural_words_))) {
      LogTrace("tone_adjuster NeuralSandhi 细分后必须轻声的词: {}",
               word_list[i]);
...
// 对 “不”进行处理，如看不懂
std::vector<std::string>
ToneAdjuster::BuSandhi(const std::string &word, const std::string &pos,
                       std::vector<std::string> &finals) {
  auto utf8_word = SplitUtf8String(word);
  // 看不懂
  if (utf8_word.size() == 3 && utf8_word[1] == "不") {
    ...
    for (int i = 0; i < utf8_word.size(); i++) {
      if (utf8_word[i] == "不" && i + 1 < utf8_word.size() &&
          finals[i + 1].back() == '4') {
        finals[i].back() = '2';
      }
    }
...
// 对 “一“ 在词语中的音调进行调整
std::vector<std::string>
ToneAdjuster::YiSandhi(const std::string &word, const std::string &pos,
                       std::vector<std::string> &finals) {
  auto utf8_word = SplitUtf8String(word);
  ...
  if (utf8_word.size() == 3 && utf8_word[1] == "一" &&
      utf8_word[0] == utf8_word[2]) {
    finals[1].back() = '5';
...
// 对词语中的三声进行特殊处理
std::vector<std::string>
ToneAdjuster::ThreeToneSandhi(const std::string &word, const std::string &pos,
                              std::vector<std::string> &finals) {
  auto utf8_word = SplitUtf8String(word);
  ...
  if (utf8_word.size() == 2 && AllToneThree(finals)) {
    ...
    if (AllToneThree(finals)) {
      ...
      finals_list.push_back(std::vector<std::string>(
          finals.begin(), finals.begin() + first_word_size));
      finals_list.push_back(std::vector<std::string>(
          finals.begin() + first_word_size, finals.end()));
      if (finals_list.size() == 2) {
        ...
        for (int i = 0; i < finals_list.size(); i++) {
          ...
          if (sub.size() == 2 && AllToneThree(sub)) {
            sub[0].back() = '2';
          } else if (i == 1 && !AllToneThree(sub) &&
                     finals_list[i][0].back() == '3' &&
                     finals_list[0][finals_list[0].size() - 1].back() == '3') {
            // TODO
            //  finals_list[0] = finals_list[0][finals_list[0].size() - 1];
            finals_list[0].back() = '2';
          }
          updated_finals.insert(updated_finals.end(), finals_list[i].begin(),
                                finals_list[i].end());
        }
...
std::vector<std::string>
ToneAdjuster::Process(const std::string &word, const std::string &pos,
                      std::vector<std::string> &finals) {
  LogDebug("tone_adjuster 输入韵母: {}", ConcatStrList(finals, "|"));

  finals = BuSandhi(word, pos, finals);
  LogDebug("tone_adjuster 处理'不'后: {}", ConcatStrList(finals, "|"));

  finals = YiSandhi(word, pos, finals);
  LogDebug("tone_adjuster 处理'一'后: {}", ConcatStrList(finals, "|"));

  finals = NeuralSandhi(word, pos, finals);
  LogDebug("tone_adjuster 处理轻声后: {}", ConcatStrList(finals, "|"));

  finals = ThreeToneSandhi(word, pos, finals);
  LogDebug("tone_adjuster 处理三声后: {}", ConcatStrList(finals, "|"));
  return finals;
}
