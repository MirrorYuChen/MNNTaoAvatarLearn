 ...
 * @Description: ChineseBert
 */
#include "Bertvits2/ChineseBert.h"
#include "Base/Logger.h"

NAMESPACE_BEGIN
ChineseBert::ChineseBert() = default;

ChineseBert::ChineseBert(const std::string &local_resource_root,
                         const std::string &mnn_mmap_dir) {
  resource_root_ = local_resource_root;

  // 读取bert token json
  LoadBertTokenFromBin(resource_root_ +
  ...
  module_.reset(Module::load(input_names, output_names, mnn_model_path.c_str(),
                             rtmgr, &mdconfig));
  LogInfo("bert模型加载成功: {}", mnn_model_path);
}
...
void ChineseBert::SaveBertTokenToBin(const std::string &filename,
                                     const bert_token &token) {
...
void ChineseBert::LoadBertTokenFromBin(const std::string &filename,
                                       bert_token &token) {
...
std::vector<int> ChineseBert::ObtainBertTokens(const std::string &text,
  ...
  LogDebug("bert words: {}", ConcatStrList(words, "|"));
...
ChineseBert::Process(const std::string &text, const std::vector<int> &word2ph,
                     const std::string &lang) {
  ExecutorScope scope(executor_);

  LogDebug("cn_bert 输入文本: {}", text);
  auto input_ids = ObtainBertTokens(text, lang);
  int token_num = input_ids.size();
  LogDebug("cn_bert 输入tokens(token_num={}):{}", std::to_string(token_num), ConcatIntList(input_ids, "|"));
  LogDebug("cn_bert 输入word2ph(size={}):{}", std::to_string(word2ph.size()), ConcatIntList(word2ph, "|"));
  ...
  LogDebug("cn_bert 输出bert feature 维度: {}", std::to_string(output_size));
  ...
  LogDebug("cn_bert output_sum: {}", std::to_string(bert_sum));
