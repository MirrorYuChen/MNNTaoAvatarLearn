...
find_initial_final(const std::string &pinyin) {
  std::string initial = "";
  std::string final = "";
  ...
  return std::make_tuple(initial,
                         final); // 如果没有找到匹配的声母，返回空字符串
}
Pinyin::Pinyin() {}
Pinyin::Pinyin(const std::string &local_resource_root) {
  auto t0 = clk::now();
  LogInfo("Pinyin 开始初始化...");
  resource_root_ = local_resource_root;

  // 读取大json速度太慢，修改为读取bin
  // auto pinyin_json_path = resource_root_ +
  // "common/text_processing_jsons/pinyin_dict.json"; auto phrase_json_path =
  // resource_root_ + "common/text_processing_jsons/phrases_dict.json";
  // 解析单字和词组的拼音数据
  // ParsePinyinJsonFile(pinyin_json_path);
  // ParsePhraseJsonFile(phrase_json_path);

  auto pinyin_bin_path =
      resource_root_ + "common/text_processing_jsons/pinyin_dict.bin";
  auto phrase_bin_path =
      resource_root_ + "common/text_processing_jsons/phrases_dict.bin";
  ...
  LogInfo("Pinyin 初始化成功, timecost: {}ms", std::to_string(d1.count()));
}

bool Pinyin::AddCustomPhrases(
...
bool Pinyin::IsPhrase(const std::string &text) {
  return phrase_map_.find(text) != phrase_map_.end();
}

bool Pinyin::IsInPrefixSet(const std::string &text) {
  return prefix_set_.find(text) != prefix_set_.end();
}
...
Pinyin::Process(const std::string &text) {
  // 获取所有拼音
  ...
  LogDebug("拼音输入文本: {}", text);
  LogDebug("拼音分词结果: {}", ConcatStrList(small_parts, "|"));

  for (auto &small_part : small_parts) {
    LogTrace("拼音开始处理|{}|", small_part);
    bool is_pinyin_valid = false;
    if (IsPhrase(small_part)) {
      LogTrace("是固定词语: {}", small_part);
      // 是否为固定词语
      is_pinyin_valid = true;
      auto cur_pinyin_list = ObtainPinyinOfPhrase(small_part);
      LogTrace("拼音: {}", ConcatStrList(cur_pinyin_list, "|"));
  ...
  // 调整输出音节形式
  LogDebug("音节调整前拼音:{}", ConcatStrList(pinyin_list, "|"));
  auto adjusted_pinyin_list = AdjustTones(pinyin_list, is_pinyin_valid_list);
  LogDebug("音节调整后拼音:{}", ConcatStrList(adjusted_pinyin_list, "|"));
  ...
  LogDebug("所有声母:{}", ConcatStrList(initial_list, "|"));
  LogDebug("所有韵母:{}", ConcatStrList(final_list, "|"));
  LogDebug(DELIMITER_LINE);
...
void Pinyin::ParseHotwordsCNFile(const std::string &hotwords_cn_json_path) {
  ...
  LogDebug("自定义热词列表:{}", ConcatStrList(hot_word_list, "|"));
}

void Pinyin::SavePinyinMapToBin(const std::string &filename,
                                const pinyin_map &pinyin_map) {
...
void Pinyin::SavePhraseMapToBin(const std::string &filename,
                                const phrase_map &phrase_map) {
  std::ofstream ofs(filename, std::ios::binary);
  if (!ofs)
    throw std::runtime_error("Unable to open file for writing");
  ...
  for (const auto &pair : phrase_map) {
    ...
    ofs.write(reinterpret_cast<const char *>(&phrases_size),
              sizeof(phrases_size));

    for (const auto &phrase : phrases) {
      size_t phrase_size = phrase.size();
      ofs.write(reinterpret_cast<const char *>(&phrase_size),
                sizeof(phrase_size));
...
void Pinyin::SavePinyinToSymbolMapToBin(
    const std::string &filename, const pinyin_to_symbol_map &pinyin_to_symbol) {
  std::ofstream ofs(filename, std::ios::binary);
  if (!ofs)
    throw std::runtime_error("Unable to open file for writing");
...
void Pinyin::LoadPinyinMapFromBin(const std::string &filename,
...
void Pinyin::LoadPhraseMapFromBin(const std::string &filename,
...
void Pinyin::LoadPinyinToSymbolMapFromBin(
    const std::string &filename, pinyin_to_symbol_map &pinyin_to_symbol) {
  std::ifstream ifs(filename, std::ios::binary);
  if (!ifs)
    throw std::runtime_error("Unable to open file for reading");
...
Pinyin::AdjustTones(const std::vector<std::string> &pinyin_list,
  ...
  for (int i = 0; i < pinyin_list.size(); i++) {
    ...
    if (is_pinyin_valid) {
      LogTrace("转音调前:{}", pinyin);
      auto adjusted_pinyin = to_tone3(pinyin);
      LogTrace("转三声后:{}", adjusted_pinyin);
      if (!ContainsNumber(adjusted_pinyin)) {
        adjusted_pinyin += "5";
        LogTrace("数字优化后:{}", adjusted_pinyin);
      }
