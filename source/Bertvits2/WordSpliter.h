...
const double MIN_FLOAT =
    std::numeric_limits<double>::lowest(); // 替代Python中的MIN_FLOAT
...
public:
  ...
  // 如for_search为false时，'我的男朋友' 结果为[我', '的',
  // '男朋友']，for_search为true时为['我', '的', '朋友', '男朋友']
  std::vector<WordPosPair> Process(const std::string &seg,
                                   bool for_search = false);

  // 对输入的分句进行分词，不过只返回分词后的词语，不返回词性
  std::vector<std::string> ProcessWoPos(const std::string &seg,
                                        bool for_search = false);
...
private:
  ...
  // 中间参数写入bin/从bin读取
  void SaveWordFreqToBin(const std::string &filename,
                         const word_freq_map &word_freq);
  void SaveWordTagToBin(const std::string &filename,
                        const word_tag_map &word_tag);
  void SaveCharStateToBin(const std::string &filename,
                          const char_state_map &char_state);
  void SaveProbEmitToBin(const std::string &filename,
                         const prob_emit_map &prob_emit);
  void SaveProbStartToBin(const std::string &filename,
                          const prob_start_map &prob_start);
  void SaveProbTransToBin(const std::string &filename,
                          const prob_trans_map &prob_trans);
  void LoadWordFreqFromBin(const std::string &filename,
                           word_freq_map &word_freq);
  void LoadWordTagFromBin(const std::string &filename, word_tag_map &word_tag);
  void LoadCharStateFromBin(const std::string &filename,
                            char_state_map &char_state);
  void LoadProbEmitFromBin(const std::string &filename,
                           prob_emit_map &prob_emit);
  void LoadProbStartFromBin(const std::string &filename,
                            prob_start_map &prob_start);
  void LoadProbTransFromBin(const std::string &filename,
  ...
  Viterbi(const std::vector<std::string> &obs);
  std::vector<WordPosPair> Cut(const std::string &sentence);
  std::vector<WordPosPair> CutDetail(const std::string &sentence);
  std::vector<WordPosPair> CutDAG(const std::vector<std::string> &sentence,
                                  const std::string &raw_s);
  std::vector<std::string> CutDAGNoHMM(const std::vector<std::string> &sentence,
                                       const std::string &raw_s);
  // 计算当前句子对应的汉字的DAG
  std::unordered_map<int, std::vector<int>>
  GetDAG(const std::vector<std::string> &sentence);
  void calc(const std::vector<std::string> &sentence,
