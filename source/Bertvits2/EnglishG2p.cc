...
EnglishG2P::EnglishG2P(const std::string &local_resource_root) {
  ...
  auto english_dict_json_path =
      resource_root_ + "common/text_processing_jsons/eng_dict.json";

  auto english_dict_bin_path =
      resource_root_ + "common/text_processing_jsons/eng_dict.bin";
...
std::string
replace_regex(const std::regex &re, const std::string &text,
...
std::string replace_punctuation(
    const std::string &text,
    const std::unordered_map<std::string, std::string> &punctuation_mapping) {
...
std::pair<std::string, int> refine_ph(const std::string &phn) {
  int tone = 0;
  std::regex tone_regex(
      "\\d$"); // Regular expression to match a digit at the end of the string

  // Check if the input string has a tone digit at the end
  if (std::regex_search(phn, tone_regex)) {
    tone = phn.back() - '0' +
           1; // Convert the last character to an integer and increment by 1
    return {ToLowercase(phn.substr(0, phn.size() - 1)),
            tone}; // Return the phoneme without the last character
  }
  return {ToLowercase(phn), tone}; // Return the original phoneme and tone=0
}

std::pair<std::vector<std::string>, std::vector<int>>
refine_syllables(const std::vector<std::vector<std::string>> &syllables) {
...
phone_data EnglishG2P::CleanedTextToSequence(const g2p_data &g2p_data_,
...
EnglishG2P::Process(SentLangPair &sent_lang) {
