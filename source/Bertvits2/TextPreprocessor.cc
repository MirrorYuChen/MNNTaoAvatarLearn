...
std::string
TextNormalizer::RemoveSpecialChineseSymbols(const std::string &text) {
...
TextPreprocessor::SplitByLang(const std::string &text,
...
// 合并相邻的中文/英文segments
std::vector<std::pair<std::string, std::string>>
TextPreprocessor::MergeAdjacent(
...
std::vector<std::vector<SentLangPair>>
TextPreprocessor::Process(std::string &text, int split_max_len) {
  // 删除空行
  ...
  for (const auto &seg : segments) {
    ...
    for (const auto &split_words : tmp) {
      ...
      // SENTENCE_SPLITOR.find(split_words.lang.back()) != std::string::npos)
      if (cur_len > split_max_len) {
