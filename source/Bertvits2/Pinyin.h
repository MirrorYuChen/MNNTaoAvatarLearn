...
enum PinyinStyle {
  ...
  NORMAL = 0,
  //: 标准声调风格，拼音声调在韵母第一个字母上（默认风格）。如： 中国 -> ``zhōng
  //guó``
  TONE = 1,
  ...
  //``ong1 uo2``
  TONE3 = 8,
  //: 声母风格，只返回各个拼音的声母部分（注：有的拼音没有声母，详见
  //`//27`_）。如： 中国 -> ``zh g``
  FINALS_TONE3 = 9,
  ...
  //: 注音风格，仅首字母。如： 中国 -> ``ㄓ ㄍ``
  BOPOMOFO_FIRST = 11,
  //: 汉语拼音与俄语字母对照风格，声调在各个拼音之后，用数字 [1-4]
  //进行表示。如： 中国 -> ``чжун1 го2``
  CYRILLIC = 12,
...
public:
   ...
   */
  bool
  AddCustomPhrases(const std::map<std::string, std::vector<std::string>> &map);
   ...
   *
   * @param text
   * 输入文本，传入的文本已经是经过预处理的，可以保证无特殊字符，但还是会有英文和数字和标点
   * @return 输入文本对应的拼音列表
   */
...
private:
  ...
  AdjustTones(const std::vector<std::string> &pinyin_list,
  ...
  // 保存中间变量到二进制bin文件
  void SavePinyinMapToBin(const std::string &filename,
                          const pinyin_map &pinyin_map);
  void SavePhraseMapToBin(const std::string &filename,
                          const phrase_map &phrase_map);
  void SavePinyinToSymbolMapToBin(const std::string &filename,
                                  const pinyin_to_symbol_map &pinyin_to_symbol);

  // 从二进制bin文件读取中间变量
  void LoadPinyinMapFromBin(const std::string &filename,
                            pinyin_map &pinyin_map);
  void LoadPhraseMapFromBin(const std::string &filename,
                            phrase_map &phrase_map);
  void LoadPinyinToSymbolMapFromBin(const std::string &filename,
                                    pinyin_to_symbol_map &pinyin_to_symbol);
