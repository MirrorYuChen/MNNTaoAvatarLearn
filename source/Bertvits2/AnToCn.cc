...
bool An2Cn::CheckInputsIsValid(const std::string &inputs) {
  bool is_valid = true;
  for (auto &c : inputs) {
    if (std::find(valid_chars_.begin(), valid_chars_.end(), c) ==
        valid_chars_.end()) {
      is_valid = false;
      break;
    }
...
std::string An2Cn::Process(const std::string &raw_inputs) {
  ...
  // 该数字包含小数点后位数
    ...
    throw std::invalid_argument(
        "[tts_an2cn] input type error, more than one dot! ");
  }
  return sign + output;
}
...
std::string An2Cn::IntegerConvert(const std::string &inputs) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (d != '0') {
      output_an +=
          NUMBER_LOW_AN2CN[d - '0'] + UNIT_LOW_ORDER_AN2CN[size - i - 1];
    } else {
      if (!(size - i - 1) % 4) {
        output_an +=
            NUMBER_LOW_AN2CN[d - '0'] + UNIT_LOW_ORDER_AN2CN[size - i - 1];
  ...
  if (utf8_output.size() > 2 && utf8_output[0] == "一" &&
      utf8_output[1] == "十") {
...
std::string An2Cn::DecimalConvert(const std::string &decimal_data) {
  ...
  if (len_decimal_data > 16) {
    LogWarn("[tts_an2cn] 注意：小数部分长度超出16位限制，将自动截取前 16 "
            "位有效精度！");
