...
WordSpliter::WordSpliter(const std::string &local_resource_root) {
  auto t0 = clk::now();
  LogInfo("WordSpliter 开始初始化...");
  resource_root_ = local_resource_root;

  LoadAllFromBin();

  auto hotwords_json_path =
      resource_root_ + "common/text_processing_jsons/hotwords_cn.json";
  ParseHotwordsCNFile(hotwords_json_path);

  auto t1 = clk::now();
  auto d1 = std::chrono::duration_cast<ms>(t1 - t0);

  LogInfo("WordSpliter 初始化完成, timecost: " + std::to_string(d1.count()) +
                 "ms");
}
...
void WordSpliter::ParseHotwordsCNFile(
    const std::string &hotwords_cn_json_path) {
  auto json_obj = LoadJson(hotwords_cn_json_path);

  std::vector<std::string> hot_word_list;
  // 由于hotwords_cn.json文件中还有别的类型的数据，这里只获取我们想要的hotword,
  // word_freq, word_type字段
...
void WordSpliter::SaveWordFreqToBin(const std::string &filename,
                                    const word_freq_map &word_freq) {
...
void WordSpliter::SaveWordTagToBin(const std::string &filename,
                                   const word_tag_map &word_tag) {
  std::ofstream ofs(filename, std::ios::binary);
  if (!ofs)
    throw std::runtime_error("Unable to open file for writing");
...
void WordSpliter::SaveCharStateToBin(const std::string &filename,
                                     const char_state_map &char_state) {
  std::ofstream ofs(filename, std::ios::binary);
  if (!ofs)
    throw std::runtime_error("Unable to open file for writing");
  ...
  for (const auto &pair : char_state) {
    ...
    ofs.write(reinterpret_cast<const char *>(&vector_size),
              sizeof(vector_size));
...
void WordSpliter::SaveProbEmitToBin(const std::string &filename,
                                    const prob_emit_map &prob_emit) {
  std::ofstream ofs(filename, std::ios::binary);
  if (!ofs)
    throw std::runtime_error("Unable to open file for writing");
  ...
  for (const auto &pair : prob_emit) {
    ...
    for (const auto &inner_pair : inner_map) {
      ...
      ofs.write(reinterpret_cast<const char *>(&inner_key_size),
                sizeof(inner_key_size));
...
void WordSpliter::SaveProbStartToBin(const std::string &filename,
                                     const prob_start_map &prob_start) {
  std::ofstream ofs(filename, std::ios::binary);
  if (!ofs)
    throw std::runtime_error("Unable to open file for writing");
...
void WordSpliter::SaveProbTransToBin(const std::string &filename,
                                     const prob_trans_map &prob_trans) {
  std::ofstream ofs(filename, std::ios::binary);
  if (!ofs)
    throw std::runtime_error("Unable to open file for writing");
...
void WordSpliter::LoadWordFreqFromBin(const std::string &filename,
                                      word_freq_map &word_freq) {
...
void WordSpliter::LoadWordTagFromBin(const std::string &filename,
                                     word_tag_map &word_tag) {
  std::ifstream ifs(filename, std::ios::binary);
  if (!ifs)
    throw std::runtime_error("Unable to open file for reading");
...
void WordSpliter::LoadCharStateFromBin(const std::string &filename,
                                       char_state_map &char_state) {
  std::ifstream ifs(filename, std::ios::binary);
  if (!ifs)
    throw std::runtime_error("Unable to open file for reading");
...
void WordSpliter::LoadProbEmitFromBin(const std::string &filename,
                                      prob_emit_map &prob_emit) {
  std::ifstream ifs(filename, std::ios::binary);
  if (!ifs)
    throw std::runtime_error("Unable to open file for reading");
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    for (size_t j = 0; j < inner_size; ++j) {
      ...
      ifs.read(reinterpret_cast<char *>(&inner_key_size),
               sizeof(inner_key_size));
...
void WordSpliter::LoadProbStartFromBin(const std::string &filename,
                                       prob_start_map &prob_start) {
  std::ifstream ifs(filename, std::ios::binary);
  if (!ifs)
    throw std::runtime_error("Unable to open file for reading");
...
void WordSpliter::LoadProbTransFromBin(const std::string &filename,
                                       prob_trans_map &prob_trans) {
  std::ifstream ifs(filename, std::ios::binary);
  if (!ifs)
    throw std::runtime_error("Unable to open file for reading");
...
void WordSpliter::SaveAllToBin() {
  auto word_freq_bin_path =
      resource_root_ + "common/text_processing_jsons/word_freq.bin";
  auto word_tag_bin_path =
      resource_root_ + "common/text_processing_jsons/word_tag.bin";
  auto char_state_bin_path =
      resource_root_ + "common/text_processing_jsons/char_state.bin";
  auto prob_emit_bin_path =
      resource_root_ + "common/text_processing_jsons/prob_emit.bin";
  auto prob_start_bin_path =
      resource_root_ + "common/text_processing_jsons/prob_start.bin";
  auto prob_trans_bin_path =
      resource_root_ + "common/text_processing_jsons/prob_trans.bin";
...
WordSpliter::Viterbi(const std::vector<std::string> &obs) {
  ...
  auto get_with_default = [](const auto &map, const std::string &key,
                             double def_val) {
    auto it = map.find(key);
    return it == map.end() ? def_val : it->second;
  };

  for (const auto &y : cur_states) { // 初始化
    V[0][y] = get_with_default(prob_start_, y, MIN_FLOAT) +
              get_with_default(prob_emit_.at(y), obs[0], MIN_FLOAT);
    mem_path[0][y] = "";
  }
  ...
  for (size_t t = 1; t < obs.size(); ++t) {
    ...
    for (const auto &y : obs_states) {
      ...
      for (const auto &y0 : prev_states) {
        double new_prob = V[t - 1][y0] +
                          get_with_default(prob_trans_.at(y0), y, MIN_INF) +
...
std::vector<WordPosPair> WordSpliter::Cut(const std::string &sentence) {
  ...
  LogTrace("wordspliter cut input: {}", sentence);

  auto result = Viterbi(ss);
  LogTrace("wordspliter cut viterbi: {},{}", std::to_string(result.first), ConcatStrList(result.second, "|"));
  ...
  for (size_t i = 0; i < ss.size(); ++i) {
    std::string pos = pos_list[i].substr(
        0, 1); // Assuming the position is the first character of the string.
...
std::vector<WordPosPair> WordSpliter::CutDetail(const std::string &sentence) {
  ...
  LogTrace("wordspliter cutdetail blocks: {}", ConcatStrList(blocks));
  ...
  for (auto &blk : blocks) {
    if (std::regex_match(blk, re_han_detail)) {
      ...
      std::sregex_token_iterator tmp_iter(blk.begin(), blk.end(),
                                          re_skip_detail, -1);
...
WordSpliter::GetDAG(const std::vector<std::string> &sentence) {
...
void WordSpliter::calc(const std::vector<std::string> &sentence,
  ...
  for (int idx = N - 1; idx >= 0; --idx) {
    std::pair<double, int> max_pair =
        std::make_pair(std::numeric_limits<double>::lowest(), idx);
    for (int x : DAG.at(idx)) {
      std::string word = HanSubstr(sentence, idx, x - idx + 1);
      double freq = std::log(
          word_freq_.find(word) != word_freq_.end() ? word_freq_[word] : 1);
...
WordSpliter::CutDAGNoHMM(const std::vector<std::string> &sentence,
  ...
  while (x < N) {
    LogTrace("==========step======================= {}", std::to_string(x));
    int y = route[x].second + 1;
    std::string l_word;
    for (size_t i = x; i < y; ++i) {
      l_word += chars[i];
    }
    LogTrace("y: {}", std::to_string(y));
    LogTrace("l_word: {}", l_word);
...
std::vector<WordPosPair>
WordSpliter::CutDAG(const std::vector<std::string> &sentence,
                    const std::string &raw_s) {
  std::vector<WordPosPair> result;
  auto DAG = GetDAG(sentence);
  for (auto &d : DAG) {
  ...
  if (!buf.empty()) {
    ...
    if (buf_utf8_list.size() == 1) {
      LogTrace("return4: {}", buf);
      result.emplace_back(buf, word_tag_[buf]);
    } else if (word_freq_.find(buf) == word_freq_.end()) {
      auto recognized = CutDetail(buf);
      result.insert(result.end(), recognized.begin(), recognized.end());
    } else {
      LogTrace("return6: {}", buf);
      result.emplace_back(buf, word_tag_[buf]);
    }
  }
...
std::vector<WordPosPair> WordSpliter::Process(const std::string &seg,
                                              bool for_search) {
  // 保存没有精细拆分的原始结果
  ...
  LogDebug("分词输入文本: {}", seg);
  LogDebug("标点分句结果: {}", ConcatStrList(parts, "|"));

  for (auto &part : parts) {
    LogDebug(DELIMITER_LINE);
    LogDebug("分词开始处理|{}|", part);
    // 如果包含汉字: 20250804 modify MatchHanInternal to MatchHan
    if (MatchHan(part)) {
      auto utf8_part = SplitUtf8String(part);
      auto fixed_part = ConcatStrList(utf8_part);
      LogDebug("cutdag 分词输入文本：{}", ConcatList(utf8_part, "|"));
      LogDebug("cutdag 分词输入文本2: {}", fixed_part);

      auto cur_raw_result = CutDAG(utf8_part, fixed_part);
      raw_result.insert(raw_result.end(), cur_raw_result.begin(),
                        cur_raw_result.end());
      LogDebug("cutdag 分词初步结果：{}", ConcatList(raw_result, "|"));
    } else {
    ...
    for (auto &r : raw_result) {
      ...
      if (hanzi_list.size() > 3) {
        for (int i = 0; i < hanzi_list.size() - 2; i++) {
          std::string cur_word =
              hanzi_list[i] + hanzi_list[i + 1] + hanzi_list[i + 2];
          bool is_word = word_freq_.find(cur_word) != word_freq_.end();
          if (is_word) {
  ...
  LogDebug("分词最终结果：{}", ConcatList(fine_result, "|"));

  return fine_result;
}

std::vector<std::string> WordSpliter::ProcessWoPos(const std::string &seg,
                                                   bool for_search) {
