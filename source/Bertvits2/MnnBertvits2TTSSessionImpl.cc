...
NAMESPACE_BEGIN
MnnBertvits2TTSSessionImpl::MnnBertvits2TTSSessionImpl(
    const std::string &local_resource_root,
    const std::string &tts_generator_model_path,
    const std::string &mnn_mmap_dir)
    : cn_g2p_(local_resource_root) {
  auto t0 = clk::now();
  LogDebug("resource_root: {}" + local_resource_root);

  resource_root_ = local_resource_root;

  // 初始化所有用到的内部处理对象
  text_preprocessor_ = TextPreprocessor();
  LogDebug("finish TextPreprocessor init.");
  auto t1 = clk::now();

  std::string cn_bert_model_path =
      resource_root_ + "common/mnn_models/chinese_bert.mnn";
  std::string cn_bert_cache_dir =
      GetOrCreateHashDirectory(mnn_mmap_dir, cn_bert_model_path);
  LogDebug("cn_bert cache_dir: {}" + cn_bert_cache_dir);

  cn_bert_model_ = ChineseBert(resource_root_, cn_bert_cache_dir);
  LogDebug("finish ChineseBert init.");
  auto t2 = clk::now();
  en_bert_model_ = EnglishBert(local_resource_root),

  en_g2p_ = EnglishG2P(resource_root_);
  LogDebug("finish EnglishG2P init.");
  auto t3 = clk::now();

  std::string tts_generator_cache_dir =
      GetOrCreateHashDirectory(mnn_mmap_dir, tts_generator_model_path);
  LogDebug("tts_generator cache_dir: {}" + tts_generator_cache_dir);
  tts_generator_ =
      TTSGenerator(tts_generator_model_path, tts_generator_cache_dir);
  auto t4 = clk::now();
  LogDebug("TTS 初始化成功");
  ...
  LogDebug("TextPreprocessor timecost: {} ms",  std::to_string(d1.count()));
  LogDebug("Load ChineseBert timecost: {} ms" + std::to_string(d2.count()));
  LogDebug("Load EnglishG2p timecost: {} ms" + std::to_string(d3.count()));
  LogDebug("Load TTSGenerator timecost: {} ms" + std::to_string(d4.count()));
}

std::tuple<phone_data, std::vector<std::vector<float>>,
           std::vector<std::vector<float>>>
MnnBertvits2TTSSessionImpl::ExtractPhoneTextFeatures(
    const std::vector<SentLangPair> &word_list_by_lang) {
  ...
  for (int i = 0; i < word_list_by_lang.size(); i++) {
    ...
    if (sent_lang.lang == "zh") {
      ...
      LogDebug("slice st: {}" + std::to_string(st));
      LogDebug("slice ed: {}" + std::to_string(ed));
      ...
      cn_bert_list.insert(cn_bert_list.end(), cn_bert_feat.begin(),
                          cn_bert_feat.end());

      en_bert_feat.resize(cn_bert_feat.size(), std::vector<float>(1024, 0.0));
      en_bert_list.insert(en_bert_list.end(), en_bert_feat.begin(),
                          en_bert_feat.end());
    } else {
      LogDebug("en");
  ...
  auto g2p_data =
      std::make_tuple(phone_data, tone_data, lang_id_data, word2ph_data);
  return std::make_tuple(g2p_data, cn_bert_list, en_bert_list);
}

std::tuple<int, Audio>
MnnBertvits2TTSSessionImpl::Process(const std::string &in_text) {
  auto t0 = clk::now();

  LogDebug("TTS input: {}" + in_text);
  ...
  LogDebug("TTS mergeLines: {}" + text);

  auto sentences = text_preprocessor_.Process(text);
  for (int i = 0; i < sentences.size(); i++) {
    LogDebug("中英文切分后句子 {}: {}", std::to_string(i), ConcatList(sentences[i], "|"));
  }

  for (int i = 0; i < sentences.size(); i++) {
    auto words_list = sentences[i];
    LogDebug("处理句子: {}" + ConcatList(words_list));
    auto [g2p_data, cn_bert_feat_data, en_bert_feat_data] =
        ExtractPhoneTextFeatures(words_list);
    Audio audio =
        tts_generator_.Process(g2p_data, cn_bert_feat_data, en_bert_feat_data);
  ...
  LogDebug("TTS timecost: {} ms, audio_duration: {} ms, rtf: {}", 
    std::to_string(timecost_in_ms), std::to_string(audio_len_in_ms), std::to_string(rtf)
  );
