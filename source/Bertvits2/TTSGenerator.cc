...
NAMESPACE_BEGIN
TTSGenerator::TTSGenerator() = default;

TTSGenerator::TTSGenerator(const std::string &tts_generator_model_path,
  ...
  module_.reset(Module::load(input_names, output_names,
                             tts_generator_model_path.c_str(), rtmgr,
                             &mdconfig));
  rtmgr->getInfo(Interpreter::SessionInfoCode::MEMORY, &mem_1);

  LogInfo("tts 模型加载成功: {}", tts_generator_model_path);
  LogInfo("### tts load memory increase : {}", mem_1 - mem_0);
}

std::vector<int16_t>
TTSGenerator::Process(const phone_data &g2p_data_,
  ...
  LogDebug("generator phones(size={}):{}", std::to_string(phones.size()), ConcatIntList(phones, "|"));
  LogDebug("generator tones(size={}):{}", std::to_string(tones.size()), ConcatIntList(tones, "|"));
  LogDebug("generator lang_ids(size={}):{}", std::to_string(lang_ids.size()), ConcatIntList(lang_ids, "|"));
  ...
  inputs[3] = MNN::Express::_Input({1, bert_feature_dim_, token_num}, NCHW,
                                   halide_type_of<float>());
  inputs[4] = MNN::Express::_Input({1, bert_feature_dim_, token_num}, NCHW,
                                   halide_type_of<float>());
  ...
  std::vector<float *> input_pointer1 = {inputs[3]->writeMap<float>(),
                                         inputs[4]->writeMap<float>()};
  ...
  LogInfo("### tts forward memory increase : {}.", mem_1 - mem_0);
  ...
  LogDebug("generator output_sum: {}", std::to_string(out_sum));
