/*
 * @Author: chenjingyu
 * @Date: 2025-08-11 09:11:39
 * @Contact: 2458006466@qq.com
 * @Description: AudioToFrameBlendShape
 */
...
NAMESPACE_BEGIN
AudioToFlameBlendShape::AudioToFlameBlendShape(
    const std::string &local_resource_root, const std::string &mnn_mmap_dir,
    const bool do_verts2flame, const int ori_fps, const int out_fps,
    const int num_exp)
    : do_verts2flame_(do_verts2flame), ori_fps_(ori_fps), out_fps_(out_fps),
      num_exp_(num_exp) {
  ...
  //    rtmgr->setExternalPath(mnn_mmap_dir,
  //    Interpreter::EXTERNAL_FEATUREMAP_DIR);

  auto audio2verts_model_path = local_resource_root_ + "audio2verts.mnn";
  audio2verts_module.reset(
      Module::load(audio2verts_input_names, audio2verts_output_names,
                   audio2verts_model_path.c_str(), rtmgr, &md_config));
  if (do_verts2flame_) {
    auto verts2flame_model_path = local_resource_root_ + "verts2flame.mnn";
    verts2flame_module.reset(
        Module::load(verts2flame_input_names, verts2flame_output_names,
                     verts2flame_model_path.c_str(), rtmgr, &md_config));
  }

  rtmgr->getInfo(Interpreter::SessionInfoCode::MEMORY, &mem_1);
  LogInfo("### audio_to_flame_blend_shape load memory increase : {}",
          mem_1 - mem_0);
}

std::vector<std::vector<float>>
AudioToFlameBlendShape::Process(const std::vector<float> &raw_audio,
                                int sample_rate) {
  ExecutorScope scope(executor_);
  ...
  audio2verts_inputs[0] =
      MNN::Express::_Input({1, audio_length}, NCHW, halide_type_of<float>());

  // 设置输入数据
  std::vector<float *> input_pointer = {
      audio2verts_inputs[0]->writeMap<float>()};
  ...
  std::vector<MNN::Express::VARP> outputs =
      audio2verts_module->onForward(audio2verts_inputs);
  rtmgr_->getInfo(Interpreter::SessionInfoCode::MEMORY, &mem_1);
  LogInfo("### audio2verts forward memory increase : {}", mem_1 - mem_0);
  ...
  if (do_verts2flame_) {
    ...
    verts2flame_inputs[0] = MNN::Express::_Input({frame_num, 5023, 3}, NCHW,
                                                 halide_type_of<float>());
    std::vector<float *> verts2flame_input_pointer = {
        verts2flame_inputs[0]->writeMap<float>()};
    ...
    // 获取verts2flame输出
    rtmgr_->getInfo(Interpreter::SessionInfoCode::MEMORY, &mem_0);
    std::vector<MNN::Express::VARP> vert2flame_outputs =
        verts2flame_module->onForward(verts2flame_inputs);
    executor_->gc(MNN::Express::Executor::FULL);

    rtmgr_->getInfo(Interpreter::SessionInfoCode::MEMORY, &mem_1);
    LogInfo("### verts2flame forward memory increase : {} \n", mem_1 - mem_0);

    auto vert2flame_out_ptr = vert2flame_outputs[0]->readMap<float>();
    flame_out_size = vert2flame_outputs[0]->getInfo()->size;
    LogInfo("===> audio2flame expr&jaw size: {}", flame_out_size);
  ...
  // 将25fps采样到20fps
  auto resample_coeffs_2d = resample_bs_params(
      coeffs_2d, static_cast<int>((static_cast<double>(coeffs_2d.size())) /
                                  ori_fps_ * out_fps_));
