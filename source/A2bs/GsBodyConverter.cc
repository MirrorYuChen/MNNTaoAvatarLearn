...
NAMESPACE_BEGIN
glm::vec3 rotateBodyPoseGlm(
    const glm::vec3 &original_pose_axis_angle,
  ...
  if (axis_norm < 1e-6f) {
    LogError("Warning: Axis for additional rotation has near-zero length. "
             "Cannot normalize. Returning original pose.");
  ...
  glm::quat additional_rotation_quat =
      glm::angleAxis(additional_angle_radians, axis_normalized);
  ...
  if (original_angle_radians < 1e-6f) {
    original_rotation_quat =
        glm::quat(1.0f, 0.0f, 0.0f, 0.0f); // Identity quaternion
  } else {
    glm::vec3 original_axis =
        glm::normalize(original_pose_axis_angle); // Safe normalization
    original_rotation_quat =
        glm::angleAxis(original_angle_radians, original_axis);
  }
...
GSBodyConverter::GSBodyConverter(const std::string &local_resource_root,
  ...
  module.reset(Module::load(input_names, output_names, mnn_model_path.c_str(),
                            rtmgr, &mdconfig));
...
std::vector<FullTypeOutput> GSBodyConverter::Process(
    const std::vector<float> &expression, const std::vector<float> &jaw_pose,
    const std::vector<BodyParamsInput> &ref_params_pool, int ref_idx) {
  ExecutorScope scope(executor_);
  std::vector<VARP> inputs(10);
  int num_frames = expression.size() / num_exp_;
  int ref_num_frames = ref_params_pool.size();

  // 初始化输入tensor
  inputs[0] = MNN::Express::_Input({num_frames, num_exp_}, NCHW,
                                   halide_type_of<float>()); // expression
  inputs[1] = MNN::Express::_Input({num_frames, 3}, NCHW,
                                   halide_type_of<float>()); // jaw_pose
  inputs[2] = MNN::Express::_Input({num_frames, 63}, NCHW,
                                   halide_type_of<float>()); // body_pose
  inputs[3] = MNN::Express::_Input({num_frames, 3}, NCHW,
                                   halide_type_of<float>()); // leye_pose
  inputs[4] = MNN::Express::_Input({num_frames, 3}, NCHW,
                                   halide_type_of<float>()); // reye_pose
  inputs[5] = MNN::Express::_Input({num_frames, 45}, NCHW,
                                   halide_type_of<float>()); // left_hand_pose
  inputs[6] = MNN::Express::_Input({num_frames, 45}, NCHW,
                                   halide_type_of<float>()); // right_hand_pose
  inputs[7] = MNN::Express::_Input({num_frames, 3}, NCHW,
                                   halide_type_of<float>()); // Rh
  inputs[8] = MNN::Express::_Input({num_frames, 3}, NCHW,
                                   halide_type_of<float>()); // Th
  inputs[9] = MNN::Express::_Input({num_frames, 100}, NCHW,
                                   halide_type_of<float>()); // pose
  ...
  // 设置输入数据
  for (int i = 0; i < num_frames * num_exp_; ++i) {
  ...
  for (int i = 0; i < num_frames; ++i) {
    ...
    for (int j = 0; j < 63; ++j) {
      ...
      //            input_pointer[2][63*i + j] =
      //            ref_params_pool[ref_idx].body_pose[j];
    }
    for (int j = 0; j < 3; ++j) {
    ...
    for (int j = 0; j < 45; ++j) {
      ...
      input_pointer[6][45 * i + j] =
          ref_params_pool[ref_idx].right_hand_pose[j];
    }
    for (int j = 0; j < 100; j++) {
  ...
  LogInfo("### gs_body_converter forward memory increase : {}", mem_1 - mem_0);
  ...
  if (outputs.empty()) {
    LogError("GSBodyConverter Empty outputs");
    return res;
  }
  ...
  for (int i = 0; i < num_frames; i++) {
    ...
    for (int j = 0; j < num_exp_ + 1; j++) {
      tmp.expr.push_back(output0_ptr[i * (num_exp_ + 1) + j]);
    }
    for (int j = 0; j < 880; j++) {
