...
AudioTo3DGSBlendShape::AudioTo3DGSBlendShape(
    ...
    const int num_exp)
    : atf(local_resource_root, mnn_mmap_dir, do_verts2flame, ori_fps, out_fps, num_exp),
      num_exp_(num_exp) {
  ...
  LogInfo("A2BSService ParseInputsFromJson execution time: {} ms",
          std::to_string(duration.count()));
}

std::vector<FLAMEOuput>
AudioTo3DGSBlendShape::ProcessFLAME(const std::vector<float> &raw_audio,
                                    int sample_rate) {
  auto t0 = clk::now();

  // 1. 先根据音频计算对应的Flame系数，得到[N, 53]的2维std::vector，
  // N为对应的数据帧 N可以根据音频长度x20得到，因为一秒对应20帧的数据
  ...
  for (int i = 0; i < num_frames; i++) {
    ...
    std::vector<float> expression(flame_bs.begin(),
                                  flame_bs.begin() + num_exp_);
  ...
  LogInfo("Audio2BS timecost: {} ms, audio_duration: {} ms, rtf:({}+{})",
          std::to_string(total_timecost_in_ms), std::to_string(audio_len_in_ms),
          std::to_string(total_rtf), std::to_string(atf_rtf),
          std::to_string(convert_rtf));
  return result;
}
