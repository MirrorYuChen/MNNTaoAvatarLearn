...
NAMESPACE_BEGIN
template <class T> class NumberIstream {
public:
  ...
  NumberIstream &operator>>(T &x) {
    if (!in_.good())
      return *this;
    in_ >> x;
    if (!in_.fail() && RemainderIsOnlySpaces())
      return *this;
...
template <typename T> bool ConvertStringToReal(const std::string &str, T *out) {
...
MergeCharactersIntoWords(const std::vector<std::string> &words) {
...
std::vector<std::string> SplitUtf8(const std::string &text) {
  ...
  while (start < end) {
    ...
    if (num_bytes == 0) {
      ...
      LogError("Invalid byte at position: {}", static_cast<int32_t>(start - begin));
      // skip this byte
      ++start;
    }
  }
...
std::string RemoveInvalidUtf8Sequences(const std::string &text,
  ...
  while (i < n) {
    ...
    if (show_debug_msg) {
      LogError("Ignore invalid utf8 sequence at pos: {}, value: {}", i, p[i]);
    }
...
std::string Gb2312ToUtf8(const std::string &text) {
  ...
  LogError("num of wchars: {}", num_wchars);
  if (num_wchars == 0) {
    return {};
  }
  ...
  MultiByteToWideChar(936, 0, text.c_str(), text.size(), wstr.data(), num_wchars);
