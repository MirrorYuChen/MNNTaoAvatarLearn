...
std::vector<std::pair<int, std::string>> UNITS = {
    {1, "十"}, {2, "百"}, {3, "千"}, {4, "万"}, {8, "亿"}};
std::unordered_map<int, int> UNITS_POS_MAP = {
    {1, 0}, {2, 1}, {3, 2}, {4, 3}, {8, 4}};

std::unordered_map<std::string, int> language_tone_start_map = {
    {"zh", 0}, {"jp", 6}, {"en", 8}};
std::unordered_map<std::string, int> language_id_map = {
    {"zh", 0}, {"jp", 1}, {"en", 2}};

std::vector<std::string> symbols = {
    "_",   "AA",   "E",  "EE", "En",  "N",    "OO",  "V",    "a",  "a:",
    "aa",  "ae",   "ah", "ai", "an",  "ang",  "ao",  "aw",   "ay", "b",
    "by",  "c",    "ch", "d",  "dh",  "dy",   "e",   "e:",   "eh", "ei",
    "en",  "eng",  "er", "ey", "f",   "g",    "gy",  "h",    "hh", "hy",
    "i",   "i0",   "i:", "ia", "ian", "iang", "iao", "ie",   "ih", "in",
    "ing", "iong", "ir", "iu", "iy",  "j",    "jh",  "k",    "ky", "l",
    "m",   "my",   "n",  "ng", "ny",  "o",    "o:",  "ong",  "ou", "ow",
    "oy",  "p",    "py", "q",  "r",   "ry",   "s",   "sh",   "t",  "th",
    "ts",  "ty",   "u",  "u:", "ua",  "uai",  "uan", "uang", "uh", "ui",
    "un",  "uo",   "uw", "v",  "van", "ve",   "vn",  "w",    "x",  "y",
    "z",   "zh",   "zy", "!",  "?",   "…",    ",",   ".",    "\"", "-",
    "SP",  "UNK"};
...
std::string ReplaceSpecificCommaNumbers(const std::string &text) {
  ...
  for (std::sregex_iterator i = begin; i != end; ++i) {
    ...
    matched.erase(std::remove(matched.begin(), matched.end(), ','),
                  matched.end());
...
//     // Create a regex pattern to match empty lines (lines with only spaces or
//     tabs) std::regex pattern(R"(^[\s\t]*\r?\n)",
...
std::string ReplaceOneMeasure(const std::string &text, const std::string &unit,
  ...
  while (pos != std::string::npos) {
    ...
    if (pos == 0 || isdigit(result[pos - 1]) ||
        (pos > 1 && result[pos - 1] == '.' && isdigit(result[pos - 2]))) {
      // 替换 "ms" 为 "毫秒"
...
std::vector<std::string> GetValue(const std::string &value_string,
  ...
  if (stripped.empty()) {
    ...
    std::string first_part =
        value_string.substr(0, value_string.size() - largest_unit);
    std::string second_part =
        value_string.substr(value_string.size() - largest_unit);
    std::vector<std::string> first_result = GetValue(first_part);
    result.insert(result.end(), first_result.begin(),
                  first_result.end()); // 添加第一部分的结果
    result.push_back(UNITS[UNITS_POS_MAP[largest_unit]].second); // 添加单位
    std::vector<std::string> second_result = GetValue(second_part);
    result.insert(result.end(), second_result.begin(),
                  second_result.end()); // 添加第二部分的结果
...
std::string Num2Str(const std::string &value_string) {
  ...
  std::string integer = (dot_pos == std::string::npos)
                            ? value_string
                            : value_string.substr(0, dot_pos);
  std::string decimal =
      (dot_pos == std::string::npos) ? "" : value_string.substr(dot_pos + 1);
...
std::string TimeNum2Str(const std::string &num_string) {
  std::string result =
      Num2Str(num_string.substr(num_string.find_first_not_of('0')));
...
RePeplaceByFunc(const std::string &text, const std::regex &pattern,
  ...
  output += std::string(
      search_start,
      text.cend()); // Append the rest of the string after the last match
...
std::string RegexReplaceDate(const std::string &sentence) {
  std::regex pattern(
      R"((\d{4}|\d{2})年((0?[1-9]|1[0-2])月)?(((0?[1-9])|((1|2)[0-9])|30|31)([日号]))?)");
...
std::string replace_temperature(const std::smatch &match) {
  ...
  result << (sign == "-" ? "零下" : "") << Num2Str(temperature)
         << (unit == "摄氏度" ? "摄氏度" : "度");
...
std::string RegexReplaceMobile(const std::string &sentence) {
  // const std::regex pattern(R"((?<!\d)((\+?86
  // ?)?1([38]\d|5[0-35-9]|7[678]|9[89])\d{8})(?!\d))");
  const std::regex pattern(
...
std::string ReplaceRange(const std::smatch &match) {
  ...
  std::string replaced_second =
      RePeplaceByFunc(second, RE_NUMBER, ReplaceNumber);
...
std::string RegexReplace2WithQuantifier(const std::string &sentence) {
  ...
  std::string quantifiers =
      "发|次|封|艘|把|目|套|段|人|所|朵|匹|张|座|回|场|尾|条|个|首|阙|阵|网|炮|"
      "顶|丘|棵|只|支|袭|辆|挑|担|颗|壳|窠|曲|墙|群|腔|砣|座|客|贯|扎|捆|刀|令|"
      "打|手|罗|坡|山|岭|江|溪|钟|队|单|双|对|出|口|头|脚|板|跳|枝|件|贴|针|线|"
      "管|名|位|身|堂|课|本|页|家|户|层|丝|毫|厘|分|钱|两|斤|担|铢|石|钧|锱|忽|"
      "(千|毫|微)克|毫|厘|(公)分|分|寸|尺|丈|里|寻|常|铺|程|(千|分|厘|毫|微)米|"
      "米|撮|勺|合|升|斗|石|盘|碗|碟|叠|桶|笼|盆|盒|杯|钟|斛|锅|簋|篮|盘|桶|罐|"
      "瓶|壶|卮|盏|箩|箱|煲|啖|袋|钵|年|月|日|季|刻|时|周|天|秒|分|小时|旬|纪|"
      "岁|世|更|夜|春|夏|秋|冬|代|伏|辈|丸|泡|粒|颗|幢|堆|条|根|支|道|面|片|张|"
      "颗|块|元|(亿|千万|百万|万|千|百)|(亿|千万|百万|万|千|百|美|)元|(亿|千万|"
      "百万|万|千|百|十|)吨|(亿|千万|百万|万|千|百|)块|角|毛|分";
...
std::string PostReplace(const std::string &sentence) {
  ...
  std::vector<std::string> to_blank_words = {
      "《", "》", "<", ">", "&", "【", "】", "{", "}",  "'", "“", "”",
      "…",  "——", "‘", "‘", "^", "#",  "@",  "_", "\\", "=", "-", "|"};
...
uint32_t Utf8ToUnicode(const std::string &utf8_str) {
  ...
  if (len == 1) {
    ...
    int offset =
        (0xFF >> (len + 1)) & utf8_str[0]; // UTF-8 的第一个字节中的有效位
    unicode_char = offset;
    for (int i = 1; i < len; ++i) {
...
bool IsChinese(uint32_t unicode_char) {
  return (unicode_char >= 0x4E00 && unicode_char <= 0x9FFF) ||
         (unicode_char >= 0x3400 && unicode_char <= 0x4DBF) ||
         ...
         (unicode_char >= 0x2B820 && unicode_char <= 0x2CEAF) ||
         (unicode_char >= 0xF900 && unicode_char <= 0xFAFF) ||
         (unicode_char >= 0x2F800 && unicode_char <= 0x2FA1F); // 扩展和兼容区间
}

// 判断字符是否为英文字母
bool IsAlphabet(char32_t char_code) {
  return (char_code >= 0x0040 && char_code <= 0x005A) ||
...
std::string FullWidthToHalfWidth(const std::string &sentence) {
  ...
  for (char32_t wc : wide_sentence) {
    if (F2H_ASCII_LETTERS.find(wc) != F2H_ASCII_LETTERS.end()) {
      ...
      result += std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t>{}
                    .to_bytes(wc);
    }
  }
...
std::vector<int16_t> ConcatAudio(std::vector<std::vector<int16_t>> audio_list,
                                 float pad_length, int sample_rate) {
...
bool HanIsDigit(const std::string &word) {
  std::vector<std::string> han_number = {"零", "一", "二", "三", "四", "五",
                                         "六", "七", "八", "九", "十"};
  return std::find(han_number.begin(), han_number.end(), word) !=
         han_number.end();
}
...
std::vector<int16_t> ConvertAudioToInt16(std::vector<float> data) {
  ...
  std::transform(data.begin(), data.end(), data.begin(),
                 [maxVal](float val) { return (val / maxVal) * 32767; });

  // 转换为 int16_t
  std::vector<int16_t> intData;
  intData.reserve(data.size());
  std::transform(
      data.begin(), data.end(), std::back_inserter(intData),
      [](float val) { return static_cast<int16_t>(std::lround(val)); });
  return intData;
}
...
DuplicateBlocks(const std::vector<std::vector<float>> &v,
  ...
  // blocksize = 312
  // assert(v.size() % bert_feat_dim == 0); // 确保 v 的大小是 bert_feat_dim
  // 的整数倍
  int token_num = v.size(); // 计算原始块的数量
                            //    assert(word2ph.size() == token_num); // 确保 p
                            //    中元素数量与块数量一致
  ...
  for (int i = 0; i < word2ph.size(); ++i) {
    ...
    for (int j = 0; j < word2ph[i]; ++j) { // 根据 p[i] 复制块 i
...
// You might need to adjust this function signature to fit your specific audio
// data structure
std::vector<int16_t> PadAudioForAtb(const std::vector<int16_t> &audio,
  ...
  int fps = 30;
  int cur_frame_num =
      std::ceil(static_cast<double>(fps) * audio.size() / sample_rate);
  int interge_frame_num = make_non_repeating_division_by_three(cur_frame_num);
  int pad_audio_elements_num =
      static_cast<int>(interge_frame_num / static_cast<double>(fps) *
                       sample_rate) -
      audio.size();
...
std::vector<int16_t> PadEmptyAudio(const std::vector<int16_t> &raw_audio_data,
                                   int sample_rate, double audio_pad_duration) {
  // Calculate the number of samples for the padding duration
  size_t pad_samples_count =
      static_cast<size_t>(sample_rate * audio_pad_duration);
  ...
  audio_data.insert(audio_data.end(), raw_audio_data.begin(),
                    raw_audio_data.end());
  audio_data.insert(audio_data.end(), empty_audio.begin(), empty_audio.end());
  return audio_data;
}
...
std::vector<std::string> SplitChineseSentence(const std::string &input,
                                              const std::string &delimiter) {
  std::vector<std::string> result;
...
json LoadJson(const std::string &json_path) {
  LogDebug("loading json file: {}", json_path);
  json json_obj;
  ...
  if (file.is_open()) {
    ...
    LogError("Failed to load json file: {}", json_path);
  }
  return json_obj;
}
...
bool MatchHanInternal(const std::string &input) {
  // 正则表达式用于匹配汉字（Unicode范围 \u4E00到\u9FD5
  LogDebug("start MatchHanInternal: {}", input);
...
std::vector<std::vector<float>>
SliceBertFeat(const std::vector<std::vector<float>> &bert_feat, int st,
              int ed) {
...
void Serialize(
  ...
  for (const auto &pair : map) {
    ...
    ofs.write(reinterpret_cast<const char *>(&outer_vector_size),
              sizeof(outer_vector_size));

    for (const auto &inner_vector : pair.second) {
      // 写入内部 vector 的大小
      size_t inner_vector_size = inner_vector.size();
      ofs.write(reinterpret_cast<const char *>(&inner_vector_size),
                sizeof(inner_vector_size));

      for (const auto &str : inner_vector) {
        // 写入字符串长度和内容
        size_t str_length = str.size();
        ofs.write(reinterpret_cast<const char *>(&str_length),
                  sizeof(str_length));
...
deserialize(const std::string &filename) {
  ...
  for (size_t i = 0; i < map_size; ++i) {
    ...
    ifs.read(reinterpret_cast<char *>(&outer_vector_size),
             sizeof(outer_vector_size));
    ...
    for (size_t j = 0; j < outer_vector_size; ++j) {
      ...
      ifs.read(reinterpret_cast<char *>(&inner_vector_size),
               sizeof(inner_vector_size));
