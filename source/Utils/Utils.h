...
// 将通用的类型定义放置到这里
typedef std::tuple<std::vector<std::string>, std::vector<int>, std::vector<int>>
    g2p_data;
...
struct SentLangPair {
  ...
  SentLangPair(const std::string &word, const std::string &lang)
      : sent(word), lang(lang) {}
...
struct WordPosPair {
  ...
  WordPosPair(const std::string &word, const std::string &flag)
      : word(word), flag(flag) {}
...
std::string StrReplaceAll(std::string &str, const std::string &from,
                          const std::string &to);
...
// 每次替换一个量词
std::string ReplaceOneMeasure(const std::string &text, const std::string &unit,
                              const std::string &cn_word);
...
// 通用的正则表达式替换函数，例如str_replace_using_re_sub("3ml",
// R"((\d+)\s?ml)", "$1 毫升")会将3ml替换为3 毫升
std::string ReReplace(const std::string &str, const std::string &pattern,
                      const std::string &replacement);

std::vector<std::string> GetValue(const std::string &value_string,
                                  bool use_zero = true);
std::string VerbalizeCardinal(const std::string &value_string);

std::string VerbalizeDigit(const std::string &value_string,
                           bool alt_one = false);
...
std::string
RePeplaceByFunc(const std::string &text, const std::regex &pattern,
...
// 将 UTF-8
// 编码的字符串拆分为单个的字符，返回字符列表，注意每个字符格式还是string
std::vector<std::string> SplitUtf8String(const std::string &utf8_str);
std::string HanSubstr(const std::vector<std::string> &hans, int begin,
                      size_t size);
...
// std::string traditional_to_simplified(const std::string& text, const
...
std::string ConcatStrList(const std::vector<std::string> &strs,
                          const std::string &delimiter = "");

// 将int数组合并成一个字符串，中间用delimiter分割，用于方便地打印数组内容
std::string ConcatIntList(const std::vector<int> &ints,
                          const std::string &delimiter = "");

// 将float数组合并成一个字符串，中间用delimiter分割，用于方便地打印数组内容
std::string ConcatFloatList(const std::vector<float> &fs,
                            const std::string &delimiter = "");
std::string Concat2dFloatList(const std::vector<std::vector<float>> &fs_2d,
                              const std::string &delimiter = "");
...
SliceBertFeat(const std::vector<std::vector<float>> &bert_feat, int st, int ed);

std::vector<int16_t> ConcatAudio(std::vector<std::vector<int16_t>> audio_list,
                                 float pad_length = 0.2,
                                 int sample_rate = 44100);

bool HanIsDigit(const std::string &word);
bool HanListContains(const std::vector<std::string> &vec,
...
std::vector<std::vector<float>>
DuplicateBlocks(const std::vector<std::vector<float>> &v,
                const std::vector<int> &word2ph, int bert_feat_dim);

//
std::vector<int16_t> PadAudioForAtb(const std::vector<int16_t> &audio,
                                    int sample_rate);

// 在音频的开始和结束部分pad特定长度的空音频
std::vector<int16_t> PadEmptyAudio(const std::vector<int16_t> &raw_audio_data,
                                   int sample_rate,
...
SplitChineseSentence(const std::string &input,
                     const std::string &delimiter = "。");

// 将句子拆分为中文和非中文部分拆分开的结果，例如"你好Hello!再见“->
// [你好，Hello!, 再见]
std::vector<std::string> SplitChineseSentenceToParts(const std::string &input,
                                                     bool merge_punc = true);
std::vector<std::string>
MergePunctuation(const std::vector<std::string> &input);
...
void LoadBinToMap(const std::string &filename,
...
// 创建子目录
std::string CreateSubDirectory(const std::string &parent_path,
                               const std::string &hash);

std::string GetOrCreateHashDirectory(const std::string &parent_dir,
                                     const std::string &filePath);

template <typename T>
std::vector<T> FlattenVector(const std::vector<std::vector<T>> &vec2d,
                             float pad_length = 0.2, int sample_rate = 44100) {
...
template <typename T>
std::vector<T> VectorSlice(const std::vector<T> &vec, size_t start,
                           size_t end) {
  // 检查边界条件和有效性
  if (start >= vec.size())
    start = vec.size();
...
template <typename T>
std::vector<T> Slice2d(const std::vector<T> &array, size_t rows, size_t cols,
  ...
  result.insert(result.end(), array.begin() + begin_idx,
                array.begin() + end_idx);
...
template <typename T>
std::string ConcatList(const std::vector<T> &list,
                       const std::string &delimiter = "|") {
  std::stringstream ss;
  ss << delimiter;
  for (auto &elem : list) {
