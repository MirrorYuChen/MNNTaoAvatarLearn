...
NAMESPACE_BEGIN
std::vector<BodyParamsInput>
ReadFramesFromBinary(const std::string &binFilePath,
  ...
  for (size_t i = 0; i < frameCount; ++i) {
    // Read frame_id
    inFile.read(reinterpret_cast<char *>(&frames[i].frame_id),
                sizeof(frames[i].frame_id));
...
void WriteFramesToBinary(const std::string &binFilePath,
  ...
  outFile.write(reinterpret_cast<const char *>(&frameCount),
                sizeof(frameCount));

  // 2. For each frame, write frame_id and each vector
  for (const auto &frame : frames) {
    // Write frame_id
    outFile.write(reinterpret_cast<const char *>(&frame.frame_id),
...
std::vector<BodyParamsInput>
ParseInputsFromJson(const std::string &jsonFilePath) {
  std::vector<BodyParamsInput> frames;
  ...
  for (const auto &item : jsonData) {
    ...
    frame.expression = flattenVector(
        item["expression"].get<std::vector<std::vector<float>>>());
    ...
    frame.body_pose =
        flattenVector(item["body_pose"].get<std::vector<std::vector<float>>>());
    frame.jaw_pose =
        flattenVector(item["jaw_pose"].get<std::vector<std::vector<float>>>());
    frame.leye_pose =
        flattenVector(item["leye_pose"].get<std::vector<std::vector<float>>>());
    frame.reye_pose =
        flattenVector(item["reye_pose"].get<std::vector<std::vector<float>>>());
    frame.left_hand_pose = flattenVector(
        item["left_hand_pose"].get<std::vector<std::vector<float>>>());
    frame.right_hand_pose = flattenVector(
        item["right_hand_pose"].get<std::vector<std::vector<float>>>());
    frame.pose =
        flattenVector(item["pose"].get<std::vector<std::vector<float>>>());
...
std::vector<float> interp_linear(const std::vector<float> &x,
...
std::vector<float> resampleAudioData(const std::vector<float> &input,
                                     unsigned int sourceSampleRate,
                                     unsigned int targetSampleRate) {
  // 初始化miniaudio重采样器
  ma_resampler_config resamplerConfig =
      ma_resampler_config_init(ma_format_f32, 1, sourceSampleRate,
                               targetSampleRate, ma_resample_algorithm_linear);
  ...
  // 执行重采样
  if (ma_resampler_process_pcm_frames(&resampler, input.data(),
                                      &inputFrameCount, output.data(),
                                      &outputFrameCount) != MA_SUCCESS) {
    // 重采样失败处理
...
std::vector<float> normalizeAudio(std::vector<float> &audio) {
  ...
  std::transform(audio.begin(), audio.end(), audio.begin(),
                 [mean, std_dev](float val) { return (val - mean) / std_dev; });
