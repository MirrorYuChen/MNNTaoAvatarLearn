/*
 * @Author: chenjingyu
 * @Date: 2025-08-06 17:11:06
 * @Contact: 2458006366@qq.com
 * @Description: ContextGraph
 */
...
void ContextGraph::Build (const std::vector<std::vector<int32_t> > &token_ids,
  ...
  if (!scores.empty()) {
    CHECK_EQ(token_ids.size(), scores.size());
  }
  if (!phrases.empty()) {
    CHECK_EQ(token_ids.size(), phrases.size());
  }
  if (!ac_thresholds.empty()) {
    CHECK_EQ(token_ids.size(), ac_thresholds.size());
  }
...
ContextGraph::ForwardOneStep (const ContextState *state, int32_t token,
  ...
  if (!node) {
    printf("Some bad things happened.");
    exit(-1);
  }
