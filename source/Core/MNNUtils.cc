...
MNN::Express::VARP GetEncoderOutFrame(MNNAllocator *allocator,
                                      ...
                                      int32_t t) {
  std::vector<int> encoder_out_shape = encoder_out->getInfo()->dim;
...
MNN::Express::VARP View(MNN::Express::VARP v) { return v; }
...
MNN::Express::VARP Repeat(MNNAllocator *allocator,
  ...
  std::vector<int> cur_encoder_out_shape = cur_encoder_out->getInfo()->dim;

  std::array<int, 2> ans_shape{hyps_num_split.back(), cur_encoder_out_shape[1]};

  MNN::Express::VARP ans = MNNUtilsCreateTensor<float>(
