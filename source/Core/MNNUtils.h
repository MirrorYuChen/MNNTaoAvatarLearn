/*
 * @Author: chenjingyu
 * @Date: 2025-08-06 17:08:20
 * @Contact: 2458006366@qq.com
 * @Description: MNNUtils
 */
#pragma once
...
#define MIRROR_READ_META_DATA_VEC(dst, src_key)                                \
  do {                                                                         \
    auto value = LookupCustomModelMetaData(meta_data, src_key, allocator);     \
    if (value.empty()) {                                                       \
      LogError("'{}' does not exist in the metadata", src_key);                \
      exit(-1);                                                                \
    }                                                                          \
                                                                               \
    bool ret = SplitStringToIntegers(value.c_str(), ",", true, &dst);          \
    if (!ret) {                                                                \
      LogError("Invalid value '{}' for '{}'", value.c_str(), src_key);         \
      exit(-1);                                                                \
    }                                                                          \
  } while (0)

// Read an integer
#define MIRROR_READ_META_DATA(dst, src_key)                                    \
  do {                                                                         \
    auto value = LookupCustomModelMetaData(meta_data, src_key, allocator);     \
    if (value.empty()) {                                                       \
      LogError("'%s' does not exist in the metadata", src_key);                \
      exit(-1);                                                                \
    }                                                                          \
                                                                               \
    dst = atoi(value.c_str());                                                 \
    if (dst < 0) {                                                             \
      LogError("Invalid value %d for '%s'", dst, src_key);                     \
      exit(-1);                                                                \
    }                                                                          \
  } while (0)

MNN::Express::VARP
MNNUtilsCreateTensor(MNNAllocator *allocator, const void *data,
                     size_t data_size, const int *shapedata, int shapeSize,
                     halide_type_t type = halide_type_of<float>());

MNN::Express::VARP
MNNUtilsCreateTensor(MNNAllocator *allocator, const int *shapedata,
                     int shapeSize,
                     halide_type_t type = halide_type_of<float>());

template <typename T>
MNN::Express::VARP MNNUtilsCreateTensor(MNNAllocator *allocator, const T *data,
                                        size_t data_size, const int *shapedata,
                                        int shapeSize) {
  return MNNUtilsCreateTensor(allocator, data, data_size, shapedata, shapeSize,
                              halide_type_of<T>());
}

template <typename T>
MNN::Express::VARP MNNUtilsCreateTensor(MNNAllocator *allocator,
                                        const int *shapedata, int shapeSize) {
  return MNNUtilsCreateTensor(allocator, shapedata, shapeSize,
                              halide_type_of<T>());
}
 ...
 *                         input_names[i].c_str()
 */
void GetInputNames(MNN::Express::Module *sess,
 ...
 */
void GetOutputNames(MNN::Express::Module *sess,
 ...
 *
 */
MNN::Express::VARP GetEncoderOutFrame(MNNAllocator *allocator,
                                      MNN::Express::VARP encoder_out,
                                      int32_t t);

std::string LookupCustomModelMetaData(const MNNMeta &meta_data, const char *key,
...
template <typename T = float> void Print1D(MNN::Express::VARP v);

// Print a 2-D tensor to stderr
template <typename T = float> void Print2D(MNN::Express::VARP v);
...
template <typename T = float> void Fill(MNN::Express::VARP tensor, T value) {
...
MNN::Express::VARP Repeat(MNNAllocator *allocator,
...
struct API CopyableOrtValue {
  ...
  /*explicit*/
  CopyableOrtValue(MNN::Express::VARP v) // NOLINT
      : value(std::move(v)) {}
