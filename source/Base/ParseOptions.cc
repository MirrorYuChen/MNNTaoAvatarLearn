...
void ParseOptions::DisableOption(const std::string &name) {
  if (argv_ != nullptr) {
    LogError("DisableOption must not be called after calling Read().");
    exit(-1);
  }
  if (doc_map_.erase(name) == 0) {
    LogError("Option {} was not registered so cannot be disabled: ",
             name.c_str());
    exit(-1);
  }
...
std::string ParseOptions::GetArg(int32_t i) const {
  if (i < 1 || i > static_cast<int32_t>(positional_args_.size())) {
    LogError("ParseOptions::GetArg, invalid index {}", i);
    exit(-1);
  }
...
static bool MustBeQuoted(const std::string &str, ShellType st) {
  ...
  CHECK_EQ(st, kBash) << "Invalid shell type.";
  ...
  for (i = 1; i < argc; ++i) {
    if (std::strncmp(argv[i], "--", 2) == 0) {
      ...
      if (!SetOption(key, value, has_equal_sign)) {
        PrintUsage(true);
        LogError("Invalid option {}", argv[i]);
        exit(-1);
      }
    } else {
      break;
...
void ParseOptions::PrintConfig(std::ostream &os) const {
  ...
  for (const auto &it : doc_map_) {
    ...
    if (bool_map_.end() != bool_map_.find(key)) {
      ...
      LogError("PrintConfig: unrecognized option {} [code error]", key.c_str());
      exit(-1);
    }
    os << '\n';
  }
...
void ParseOptions::ReadConfigFile(const std::string &filename) {
  ...
  if (!is.good()) {
    LogError("Cannot open config file: {}", filename);
    exit(-1);
  }
  ...
  while (std::getline(is, line)) {
    ...
    if (line.substr(0, 2) != "--") {
      LogError("Reading config file {}: line {} does not look like a line "
    ...
    if (!SetOption(key, value, has_equal_sign)) {
      PrintUsage(true);
      LogError("Invalid option {} in config file {}: line {}", line, filename,
               line_number);
      exit(-1);
    }
  }
...
void ParseOptions::SplitLongArg(const std::string &in, std::string *key,
  ...
  CHECK(in.substr(0, 2) == "--") << in; // precondition.
  ...
  if (pos == std::string::npos) { // we allow --option for bools
    ...
    *has_equal_sign = false;
    ...
    PrintUsage(true);
    LogError("Invalid option (no key): {}", in);
    exit(-1);
...
bool ParseOptions::SetOption(const std::string &key, const std::string &value,
  ...
  if (bool_map_.end() != bool_map_.find(key)) {
    ...
    *(double_map_[key]) = ToDouble(value);
    ...
    if (!has_equal_sign) {
      LogError("Invalid option --{} (option format is --x=y).", key);
      exit(-1);
    }
...
bool ParseOptions::ToBool(std::string str) const {
  ...
  PrintUsage(true);
  LogError("Invalid format for boolean argument [expected true or false]: {}",
           str);
  exit(-1);
...
int32_t ParseOptions::ToInt(const std::string &str) const {
  ...
  if (!ConvertStringToInteger(str, &ret)) {
    LogError("Invalid integer option \"{}\"", str);
    exit(-1);
  }
  return ret;
}
...
int64_t ParseOptions::ToInt64(const std::string &str) const {
  int64_t ret = 0;
  if (!ConvertStringToInteger(str, &ret)) {
    LogError("Invalid integer int64 option \"{}\"", str);
    exit(-1);
  }
  return ret;
}
...
float ParseOptions::ToFloat(const std::string &str) const {
  ...
  if (!ConvertStringToReal(str, &ret)) {
    LogError("Invalid floating-point option \"{}\"", str);
    exit(-1);
  }
  return ret;
}
...
template void ParseOptions::RegisterStandard(const std::string &name, bool *ptr,
                                             const std::string &doc);
template void ParseOptions::RegisterStandard(const std::string &name, int *ptr,
                                             const std::string &doc);
...
template void ParseOptions::RegisterCommon(const std::string &name, bool *ptr,
                                           const std::string &doc,
                                           bool is_standard);
template void ParseOptions::RegisterCommon(const std::string &name, int *ptr,
                                           const std::string &doc,
                                           bool is_standard);
